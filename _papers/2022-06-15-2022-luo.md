---
title: "Luo, J., Yang, M., & Wang, L. (2022). Learned irrelevant stimulus-response associations and proporti"
short_title: "Luo, J., Yang, M., & Wang, L. (2022). Learned irre"
category: paper
date: "2022-06-15"
tags:
  - "ddm"
...
# Luo, J., Yang, M., & Wang, L. (2022). Learned irrelevant stimulus-response associations and proportion congruency effect: A diffusion model account. Journal of Experimental Psychology: Learning, Memory, and Cognition. https://doi.org/10.1037/xlm0001158

[JEP : 华南师范大学心理学院王凌课题组揭示无关刺激反应联结学习是比例一致性效应的一般机制](https://mp.weixin.qq.com/s/5m-8ikam5xqkeOPu8lK7Cg)

[Luo et al. - 2022 - Learned irrelevant stimulus-response associations .pdf](Luo,%20J%20,%20Yang,%20M%20,%20&%20Wang,%20L%20(2022)%20Learned%20irrele/Luo_et_al._-_2022_-_Learned_irrelevant_stimulus-response_associations_.pdf)
...
![图1：调节starting point（*z*，第一行）或drift criterion（*d*，第二行）的模拟结果。纵坐标为Simon效应（不一致试次减一致试次），横坐标为反应时。中性条件的delta function是由Ulrich et al.（2015）中Simon任务的最佳拟合参数生成的。多数一致和多数不一致条件则是在这组最佳拟合参数的基础上，根据习得的无关刺激反应联结对starting point或drift criterion进行调节。](Luo,%20J%20,%20Yang,%20M%20,%20&%20Wang,%20L%20(2022)%20Learned%20irrele/Untitled.jpeg)
...
![图3：行为结果。（A）反应时；（B）错误率；（C）反应时的Simon效应；（D）错误率的Simon效应。25/75表示25%的一致试次和75%的不一致试次；75/25表示75%的一致试次和25%的不一致试次。Con表示一致，Inc表示不一致](Luo,%20J%20,%20Yang,%20M%20,%20&%20Wang,%20L%20(2022)%20Learned%20irrele/Untitled.png)

图3：行为结果。（A）反应时；（B）错误率；（C）反应时的Simon效应；（D）错误率的Simon效应。25/75表示25%的一致试次和75%的不一致试次；75/25表示75%的一致试次和25%的不一致试次。Con表示一致，Inc表示不一致

![图2：在多数一致条件下调节注意相关参数（增大任务相关信息的积累速率[*μc*]和减小无关信息的影响[*A*]）的模拟结果。左上角的图作为基线，由Ulrich et al.（2015）中Simon任务的最佳拟合参数生成。](Luo,%20J%20,%20Yang,%20M%20,%20&%20Wang,%20L%20(2022)%20Learned%20irrele/Untitled%201.jpeg)

图2：在多数一致条件下调节注意相关参数（增大任务相关信息的积累速率[*μc*]和减小无关信息的影响[*A*]）的模拟结果。左上角的图作为基线，由Ulrich et al.（2015）中Simon任务的最佳拟合参数生成。

![图4：Delta function的观测值和模型预测值。模型1-3分别表示调节starting point、drift criterion和注意相关参数。Con表示一致，Inc表示不一致，MC表示多数一致，MI表示多数不一致。](Luo,%20J%20,%20Yang,%20M%20,%20&%20Wang,%20L%20(2022)%20Learned%20irrele/Untitled%202.jpeg)
